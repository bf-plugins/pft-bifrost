...
# First DADA buffer
    ...
    with bf.block_scope(fuse=False, gpu=0):
        ...
        b_gpu_incoh = bf.blocks.reduce(b_gpu, axis='antenna')
    ...
    b_cpu_incoh = bf.blocks.copy(b_gpu_incoh, space='cuda_host')
    b_cpu_incoh = bf.blocks.transpose(b_cpu_incoh, ['time', 'fine_time', 'antenna', 'pol', 'freq'])
    b_cpu_incoh0 = bf.views.merge_axes(b_cpu_incoh, 'time', 'fine_time')
    
    b_cpus = {}
    for ant_id in ant_ids:
        b_cpus[ant_id] = ExtractAntennaBlock2(b_cpu0, ant_id=ant_id, nchan=args.numchan)
        b_cpus[ant_id] = RenameBlock(b_cpus[ant_id], 'bf0_a%i' % ant_id)
    b_cpu_incoh0 = ExtractAntennaBlock2(b_cpu_incoh0, ant_id=0, nchan=args.numchan)
    b_cpu_incoh0 = RenameBlock(b_cpu_incoh0, 'bf_incoh')

    for ant_id, blk in b_cpus.items():
         bf.blocks.write_sigproc(blk, path=outdir)
    bf.blocks.write_sigproc(b_cpu_incoh0, path=outdir)
