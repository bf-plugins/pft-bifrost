...
def dada_dict_to_bf_dict(hdr):
    ...
    assert nchan_per_sub == 32

    nant_per_snap = int(nant / nsnap)
    assert nant_per_snap == 6

    db_size      = int(hdr['DADA_BUF_SIZE'])
    nheap        = int(db_size / int(hdr['RESOLUTION']))
    telescope_id = 4

    bw           = float(hdr['BW'])
    bw_chan      = float(hdr['CHAN_BW'])
    f_mhz0       = float(hdr['FREQ']) - (bw / 2)
    ...
    pico_seconds = float(hdr['PICOSECONDS'])
    utc_str = hdr['UTC_START']
    utc_str = utc_str[:10] + " " + utc_str[11:]
    ts = Time(utc_str, format='iso').unix + (pico_seconds / 1e12)
    dt = float(1.0/bw_chan) * 1e-6
    name = 'bf_{ts}'.format(ts=Time(utc_str, format='iso').isot)

    h = {
        '_tensor': {'dtype':  dtype,
                     'shape':  [-1,     nheap,  nsubband,  nsnap,  nframe,  nchan_per_sub,  nant_per_snap, npol],
                     'labels': ['time', 'heap', 'subband', 'snap', 'frame', 'freq',         'station', 'pol'],
                     'units':  ['s',    's',    'MHz',     '',     's',     'MHz',          '',        ''],
