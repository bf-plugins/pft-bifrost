...
if __name__ == "__main__":
    ...
    p.add_argument('-n', '--now', action='store_true', default=False,
                   help='Capture now.')
    p.add_argument('-o', '--outriggers', action='store_true', default=False,
                   help='Power up outriggers')
    args = p.parse_args()
    ...
    print("Outrigger power control:  {}".format(args.outriggers))
    ...
    once = True
    #os.system("ssh -i ~/.ssh/id_rsa_pi pi@ut2d-s \"/mnt/utmost2d/module_power_control/scripts/man_bat_ctrl.py -f0\"")
    #os.system("ssh -i ~/.ssh/id_rsa_pi pi@ut2d-n \"/mnt/utmost2d/module_power_control/scripts/man_bat_ctrl.py -f0\"")
    ...
    try:
        while once:
            ...
            if args.beamformer:
                if args.outriggers:
                    print("Turning on outrigger feeds")
                    os.system("ssh -i ~/.ssh/id_rsa_pi pi@ut2d-s \"/mnt/utmost2d/module_power_control/scripts/man_bat_ctrl.py -f1\"")
                    os.system("ssh -i ~/.ssh/id_rsa_pi pi@ut2d-n \"/mnt/utmost2d/module_power_control/scripts/man_bat_ctrl.py -f1\"")
            ...
            if (not args.now) and (sleep_dur > 0):
...
#                #time.sleep(0)
            ...
            if args.capture:
                ...
                #    time.sleep(1)
                # now capture data
                print("{} {} {}".format(next_src, ra_str_next, dec_str_next))
                os.system("./tcp_start_sync.py {} {} {}".format(next_src, ra_str_next, dec_str_next))
            ...
            if args.capture:
                ...
                os.system("./tcp_stop_sync.py")
                data_capture_active = False
    
            if args.beamformer:
                if args.outriggers:
                    print("Turning off outrigger feeds")
                    os.system("ssh -i ~/.ssh/id_rsa_pi pi@ut2d-s \"/mnt/utmost2d/module_power_control/scripts/man_bat_ctrl.py -f0\"")
                    os.system("ssh -i ~/.ssh/id_rsa_pi pi@ut2d-n \"/mnt/utmost2d/module_power_control/scripts/man_bat_ctrl.py -f0\"")
